...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round((df.loc[(df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == '>50K')].shape[0] / df.loc[df['hours-per-week'] == df['hours-per-week'].min()].shape[0]) * 100,1)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = pd.DataFrame(df.loc[df['salary'] == '>50K'].groupby('native-country').size()  / df.groupby('native-country').size(),columns=['percent_rich']).idxmax().values[0]

    highest_earning_country_percentage = round((pd.DataFrame(df.loc[df['salary'] == '>50K'].groupby('native-country').size()  / df.groupby('native-country').size(),columns=['percent_rich']).max().values[0])*100,1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = top_IN_occupation = pd.DataFrame(df.loc[(df['salary'] == '>50K') & (df['native-country'] == 'India')]).groupby('occupation').size().idxmax()
